...
// NewRsyncLogStream creates a LogStreams instance that will stream rsync progress for the specified PVC.
// It uses restCfg to create Kubernetes clients and podLabels to locate the rsync pod.
// If output is non-empty the returned stream will persist progress JSON to that file path; otherwise no file is written.
...
// writeProgressToFile writes the Progress object p as indented JSON to the file at path o,
// creating the file if it does not exist.
// It returns any file I/O error encountered while creating or writing the file.
...
// addDataSize adds two dataSize values, normalizing units and returning their sum as a new *dataSize.
// If b is nil, addDataSize returns nil. Supported units are "bytes", "K", "M", "G", and "T";
// when units differ, the function converts values to a common unit using decimal power scaling and
// returns the combined value in the chosen unit. Inputs are not modified. If either unit is not
// recognized, the returned dataSize will contain zero value and an empty unit.
...
// NewProgress creates a new Progress for the provided PVC name with FailedFiles and Errors initialized and startedAt set to the current time.
...
// newDataSize parses a size string into a *dataSize value.
// 
// The input may contain a numeric value optionally followed by a unit (e.g. "12.3M", "1024", "1.5G"). If no unit is present the unit defaults to "bytes". Returns nil if the string cannot be parsed as a numeric value.
...
// parseRsyncLogs parses raw rsync log text into a Progress structure and returns any trailing incomplete line.
// 
// The returned Progress contains parsed fields such as transfer percentage, transferred data and rate,
// total and transferred file counts, failed files, miscellaneous errors, and retry count when present.
// The second return value is any unprocessed trailing data (a partial line without the terminating newline)
// that should be preserved and prepended to the next chunk of logs; it is empty when there is no partial line.
...
// It returns an error if the pod list retrieval fails or if the polling helper reports an error.
...
// getFinalPodStatus retrieves the rsync container's exit code (if available) and the last 35 lines of its logs for the given pod.
// 
// It polls the pod status up to five times (or until a terminated state with an exit code is observed) to capture a final exit code.
// After polling it requests and returns the last 35 lines of the "rsync" container logs.
// Returns a pointer to the exit code (nil if not found), the collected log text (empty string on failure to read logs), and any error encountered while querying the pod or streaming logs.

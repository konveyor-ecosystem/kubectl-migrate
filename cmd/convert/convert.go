...
// NewConvertOptions creates a Cobra command for the "convert" subcommand and configures logging and CLI flags using the provided IOStreams.
// 
// It initializes a ConvertOptions instance (including a logger that writes to streams.Out), registers command flags, and wires the command's
// RunE to invoke Complete and Run on the configured options.
// 
// The streams parameter supplies the command's input/output streams and is used as the logger output destination.
// 
// It returns the configured *cobra.Command ready to be added to a CLI root command.
...
// addFlagsForConvertOptions adds command-line flags to cmd that populate the provided ConvertOptions.
// Flags added: --source-context, --namespace (-n), --resource (-r), --search-registries (-s), --insecure-registries,
// --block-registries, --export-dir (-e), and --debug.

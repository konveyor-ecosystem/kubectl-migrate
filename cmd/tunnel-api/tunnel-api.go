...
// NewTunnelAPIOptions constructs a cobra command named "tunnel-api" configured to set up an OpenVPN tunnel between a source (on-prem) and destination (cloud) cluster.
// The provided IOStreams are used for command input/output; the command's execution sequence calls Complete, Validate, then Run and exposes flags for contexts, namespace, images, and proxy settings.
...
// addFlagsForTunnelAPIOptions adds CLI flags to cmd that populate the provided TunnelAPIOptions.
// The registered flags control source/destination kube contexts, target namespace, container images
// for source and destination, and optional HTTP proxy settings (host, port, username, password).
// Image flags default to quay.io/konveyor/openvpn:latest if not provided; proxy port defaults to 3128
// when not specified.

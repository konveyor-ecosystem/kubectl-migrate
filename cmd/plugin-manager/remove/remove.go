...
// NewRemoveCommand returns a configured Cobra command for removing an installed plugin by name.
// The command is configured with usage "remove <name>" and runs the Options lifecycle (Complete,
// Validate, Run) when executed. In PreRun it binds command flags to Viper and unmarshals them into
// the command Options and provided global flags parameter `f`.
//
// f is the global flags instance used to populate the command's Options. The function returns the
// configured *cobra.Command ready to be added to a parent command.

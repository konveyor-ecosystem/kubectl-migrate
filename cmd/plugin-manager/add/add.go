...
// NewAddCommand creates the Cobra "add" command for installing plugins.
// It initializes command options with the provided global flags, binds and unmarshals flags in PreRun,
// and wires command execution to invoke Complete, Validate (errors are logged), and Run in sequence.
// The returned *cobra.Command is configured with the add command flags and ready to be registered.
...
// addFlagsForOptions registers the command-line flags used by the add command and binds them to o.
// It defines the --version flag (bound to o.Version) to select a specific plugin version and the --global flag (bound to o.Global) to request a global installation location.
...
// downloadBinary downloads a plugin binary from a remote URL or copies it from a local path and writes it to the given directory.
//
// downloadBinary determines whether the provided url is a remote URL or a local filesystem path. It fetches or opens the source, creates the target directory if necessary, creates or truncates the destination file, writes the binary contents, and syncs the file to disk. The destination file is created with mode 0777. An informational log entry is emitted on success.
//
// It returns an error if any network, I/O, or filesystem operation fails.

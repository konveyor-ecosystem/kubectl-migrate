...
// NewListCommand constructs the "list" Cobra command which enumerates available plugins.
// The command wires flag binding to viper, unmarshals configuration into the command options,
// registers list-specific flags, and runs the command lifecycle (Complete -> Validate -> Run).
// It accepts global flags via the provided *flags.GlobalFlags and returns a configured *cobra.Command.
...
// addFlagsForOptions registers CLI flags used by the list command on the provided Cobra command.
// 
// It adds flags to control listing behavior:
// - --installed: list installed plugins.
// - --params: when used with --name, show metadata for all versions of the named plugin; takes precedence over --versions.
// - --name: specifies the plugin to inspect for --params or --versions.
// - --versions: when used with --name, list all available versions of the named plugin.
...
// printInstalledInformation prints a table for each installed plugin showing its name,
// version, and formatted optional fields.
...
// groupInformationForPlugins groups plugin versions by plugin name, aggregates their versions and short descriptions, and prints the resulting summary.
//
// pluginsMap maps repository identifiers to slices of plugin.PluginVersion; the function consolidates all versions for each plugin name across the provided repositories and delegates rendering to printInformation.
...
// printInformation prints a table for each available plugin that has a non-empty Name.
// Each table shows the plugin Name, ShortDescription, and a comma-separated list of AvailableVersions
// taken from the provided availablePlugins map.
...
// printParamsInformation prints detailed parameter and metadata tables for each plugin version
// in the provided map keyed by repository name. For every plugin version it renders a table
// showing the plugin's name, short description, full description, the version string, and any
// optional fields.
...
// getOptionalFields formats a slice of OptionalFields into a multiline string.
// 
// Each OptionalFields entry is rendered as a sequence of lines showing field
// names and values; the first field of an entry is prefixed with "- " and any
// subsequent fields of the same entry are prefixed with two spaces. Lines are
// joined with newline characters. Returns an empty string if the input slice
// is empty.
...
// printTable renders the provided rows and columns as a table to stdout.
// 
// The `data` argument is a slice of rows, where each row is a slice of column strings.
// Text wrapping is disabled; the table is written directly to standard output.

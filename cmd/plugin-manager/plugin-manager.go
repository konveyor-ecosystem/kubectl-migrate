...
import (
	...
	"github.com/konveyor-ecosystem/kubectl-migrate/cmd/plugin-manager/add"
	"github.com/konveyor-ecosystem/kubectl-migrate/cmd/plugin-manager/list"
	"github.com/konveyor-ecosystem/kubectl-migrate/cmd/plugin-manager/remove"
...
// NewPluginManagerCommand creates the root "plugin-manager" Cobra command that manages plugins.
// The returned command wires persistent flags from the provided global flags, binds flags to Viper in PreRun, executes option completion, validation and execution in RunE, and registers the list, add, and remove subcommands.
...
// addFlagsForOptions registers persistent CLI flags on cmd to populate o.
// It defines the --plugin-dir flag (shorthand -p) which defaults to $HOME plus
// plugin.DefaultLocalPluginDir, and the --repo flag (no shorthand) which defaults
// to the empty string.

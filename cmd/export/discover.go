...
// writeResources writes each object's YAML representation to disk under the provided
// directories, using clusterResourceDir for cluster-scoped objects (no namespace) and
// resourceDir for namespaced objects.
// It skips resources with an empty Kind. For each object it creates a file named by
// getFilePath(obj), marshals the object's contents to YAML, writes the YAML to the file,
// and closes the file. Any errors encountered during file creation, marshaling, writing,
// or closing are collected and returned.
...
// writeErrors writes each groupResourceError as a YAML file named "<resourceName>.yaml" into failuresDir.
// It skips entries whose APIResource Kind is empty and returns a slice of any errors encountered while creating,
// marshaling, writing, or closing the files.
...
// Example result: "Deployment_apps_v1_default_my-app.yaml".
...
// resourceToExtract determines which API resources should be extracted and retrieves their objects,
// returning the resources that contain at least one object and any errors encountered while listing.
//
// The function filters resources by: presence of supported verbs, exclusion of Event kind, admission
// based on clusterScopedRbac and resource scope, and membership in the preferred API version for the
// resource's API group. For each admitted and preferred resource it attempts to list objects using
// the provided dynamic client, namespace, and labelSelector; successful listings with one or more
// items are returned as populated groupResource entries. Any listing failures are recorded as
// groupResourceError entries and returned alongside the discovered resources.
...
// isAdmittedResource reports whether the given API resource is admitted for extraction
// based on cluster RBAC scope and the resource's scope. For cluster-scoped resources it
// returns true only if clusterScopedRbac is true and the resource is recognized as
// cluster-scoped; namespaced resources are always admitted.
...
// getObjects retrieves the objects for the given groupResource using the provided dynamic client,
// scoped to the optional namespace and filtered by the optional labelSelector.
// For namespaced resources the namespace argument is used; for cluster-scoped resources it is ignored.
// The function returns the collected objects as an UnstructuredList, or an error if the list or subsequent
// item processing fails. For resources named "imagestreamtags" or "imagetags" items are fetched individually
// and returned as an UnstructuredList.
...
// iterateItemsByGet iterates over a runtime list, fetches each item's full object via GET, and returns the collected UnstructuredList.
// 
// The function expects each list entry to be an *unstructured.Unstructured; it returns an error if an entry has a different type
// or if any GET request fails. On success, the returned UnstructuredList contains the objects retrieved from the server.
...
// iterateItemsInList iterates over a runtime.Object list and collects each item into an UnstructuredList.
// It returns the assembled UnstructuredList or an error if the list cannot be processed or contains items
// that are not *unstructured.Unstructured.

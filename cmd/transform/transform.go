...
// NewTransformCommand creates and configures the "transform" Cobra command which generates plugin-driven transformations for exported resources and saves the results in a transform directory.
// The provided GlobalFlags f are used to initialize command options; the command binds flags with viper, registers transform-specific and persistent flags, and adds subcommands for optionals and plugin listing. When executed, the command runs the options' Complete, Validate, and Run lifecycle.
...
// addFlagsForOptions registers command-line flags on cmd and binds their values to the provided Flags struct.
// It configures export-dir, transform-dir, ignored-patches-dir, plugin-priorities, optional-flags, and persistent
// plugin-related flags (plugin-dir and skip-plugins) used by the transform command and its subcommands.
...
// optionalFlagsToLower returns a new map with all keys from inFlags converted to lowercase.
// Values are preserved; if multiple original keys normalize to the same lowercase key,
// the last value encountered for that key is kept.

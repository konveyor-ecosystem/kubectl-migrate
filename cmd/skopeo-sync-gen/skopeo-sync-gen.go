...
// NewSkopeoSyncGenCommand creates a `skopeo-sync-gen` cobra command that generates a skopeo sync source YAML and writes it to stdout.
// It accepts global CLI flags via the provided `*flags.GlobalFlags` and registers the command flags `--export-dir`/`-e` (default "export"), `--registry-url` (required), and `--internal-registry-url` (default InternalRegistryDefault).
// The command binds flags to viper in PreRun and executes the Options lifecycle (Complete, Validate, Run) in RunE.
...
// shouldAddImageStream reports whether any tag item's DockerImageReference begins with the given internalRegistryURL.

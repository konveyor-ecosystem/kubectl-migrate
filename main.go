...
// main sets up and runs the kubectl-migrate root command, wiring global flags and subcommands.
// It constructs the command metadata, attaches subcommands for export, transfer-pvc, tunnel-api,
// convert, transform, skopeo-sync-gen, apply, plugin-manager, version, and runfn, then executes the
// command and exits with status 1 if execution returns an error.
func main() {
	f := &flags.GlobalFlags{}
	root := cobra.Command{
		Use:   "kubectl-migrate",
